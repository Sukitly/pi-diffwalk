...
- folding with a decision model: every attention reason in the policy and the fold-by-default rule for every other kind, the unresolved-comment gate that skips the model, unit text rendering with bounded context, the HTTP client against a fake fetch including timeout and malformed answers, the settings and key resolution, and the session path where a judge folds or marks each unit, a failing judge degrades once, and a misconfiguration warns
