...
test("captures global and project rules once in precedence order", async () => {
  const harness = createHarness({
    globalRulesResult: {
      status: "loaded",
      rules: {
  ...
  assert.ok(
    firstPrompt.indexOf("Review security boundaries before callers.") <
      firstPrompt.indexOf("Keep behavioral tests with their implementation."),
  );
...
test("applies global rules when project trust suppresses project rules", async () => {
...
test("keeps global rules when a changed project rules file is ignored", async () => {
  const notifications: string[] = [];
  const harness = createHarness({
    globalRulesResult: {
      status: "loaded",
...
test("loads one rules source when the other is unavailable", async () => {
  const globalNotifications: string[] = [];
  const projectOnly = createHarness({
    globalRulesResult: {
      status: "unavailable",
  ...
  await projectOnly.command("", commandContext("tui", globalNotifications));
  assert.match(
    projectOnly.sentMessages[0] ?? "",
    /Review project contracts first\./,
  );
  assert.match(globalNotifications[0] ?? "", /Continuing without those rules/);
  ...
  await globalOnly.command("", commandContext("tui", projectNotifications));
  assert.match(
    globalOnly.sentMessages[0] ?? "",
    /Review global contracts first\./,
  );
  assert.match(projectNotifications[0] ?? "", /Continuing without those rules/);
});

test("keeps global rules when the project rules loader rejects", async () => {
  const notifications: string[] = [];
  const harness = createHarness({
    globalRulesResult: {
      status: "loaded",
